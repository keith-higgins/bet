globalThis.__timing__.logStart('Load chunks/build/useChallengeData-DZctN6nO');import { createClient } from '@supabase/supabase-js';
...
function toUiRound(row) {
  var _a;
  const dbBet = (_a = row.bets) == null ? void 0 : _a[0];
  return {
    id: row.id,
    ...
    status: row.status,
    bets: [{ id: (dbBet == null ? void 0 : dbBet.id) || null, bettor: "Keith", bettorId: (dbBet == null ? void 0 : dbBet.bettor_id) || row.bettor_id, type: (dbBet == null ? void 0 : dbBet.bet_type) || "Accumulator", stake: Number((dbBet == null ? void 0 : dbBet.stake) || row.stake), status: (dbBet == null ? void 0 : dbBet.status) || "pending", actualReturn: (dbBet == null ? void 0 : dbBet.actual_return) == null ? null : Number(dbBet.actual_return), selections: ((dbBet == null ? void 0 : dbBet.bet_selections) || []).map((selection) => {
      var _a2;
      return { id: selection.id, matchId: ((_a2 = selection.matches) == null ? void 0 : _a2.provider_match_id) || selection.match_id, match: selection.matches ? `${selection.matches.home_team} v ${selection.matches.away_team}` : "", market: toUiMarket(selection.market), pick: selection.pick, odds: Number(selection.odds), status: selection.status };
    }) }]
  };
}
function useChallengeData() {
  ...
  async function saveBetToDatabase({ roundId, bettorId, bet, legs, matches }) {
    ...
    try {
      ...
      const matchRows = legs.map((leg, index) => {
        ...
        return { provider: (source == null ? void 0 : source.provider) || "manual", provider_match_id: leg.matchId || (source == null ? void 0 : source.id) || manualId, home_team: (source == null ? void 0 : source.home) || home || label, away_team: (source == null ? void 0 : source.away) || away || "Selection", home_score: (_a = source == null ? void 0 : source.homeScore) != null ? _a : null, away_score: (_b = source == null ? void 0 : source.awayScore) != null ? _b : null, status: (source == null ? void 0 : source.status) || "scheduled", minute: (source == null ? void 0 : source.minute) || null };
      });
      ...
      const selectionRows = legs.map((leg, index) => {
        var _a;
        const label = String(leg.match || "Unlinked match");
        const manualId = `manual-${index}-${label}`.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
        return { bet_id: savedBet.id, match_id: matchIds[leg.matchId || ((_a = matches[index]) == null ? void 0 : _a.id) || manualId], market: toDatabaseMarket(leg.market), pick: leg.pick, odds: Number(leg.odds), status: "pending" };
  ...
  async function createInitialChallenge() {
    ...
    try {
      ...
      const { error: memberError } = await client.from("challenge_members").insert({ challenge_id: challengeId, user_id: user.id, display_name: ((_a = user.email) == null ? void 0 : _a.split("@")[0]) || "Keith", role: "owner" });
      if (memberError) throw memberError;
      const deadline = new Date(Date.now() + 7 * 24 * 60 * 60 * 1e3).toISOString();
...
export { useChallengeData as u };;globalThis.__timing__.logEnd('Load chunks/build/useChallengeData-DZctN6nO');
//# sourceMappingURL=useChallengeData-DZctN6nO.mjs.map
