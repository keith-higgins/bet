...
function normalizeEventMarkets(markets, eventId) {
  return Object.values(markets)
    ...
    .map((market) => ({
      ...
      // Paddy Power's own stable id for the market (e.g. TO_SCORE) — the same market
      // can be worded differently across matches/competitions ("Anytime Goalscorer"
      // vs "Player To Score" are both TO_SCORE), so this is what categorization and
      // settlement should match on, not the display name.
...
async function fetchViaBrowser(pageUrl, browser) {
...
export async function fetchPage(pageUrl, browser) {
  const payload = await fetchViaBrowser(pageUrl, browser)
  return {
