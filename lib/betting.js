...
// Case/punctuation-insensitive lookup so near-matches (different capitalization, a
// missing "?", OCR'd text) still resolve to the same category as an exact label.
const NORMALIZED_MARKET_DATABASE_VALUES = Object.fromEntries(
  Object.entries(MARKET_DATABASE_VALUES).map(([label, value]) => [
...
export function resolveMarketDatabaseValue(marketLabel) {
  ...
  const normalized = NORMALIZED_MARKET_DATABASE_VALUES[normalizeMarketLabel(label)]
