...
export class FootballProvider {
  ...
  filterFixtures(events, query = '') {
    ...
    // punctuation-stripped query (e.g. "paris saint germain").
  ...
  async getFixtures(query = '') {
    ...
    // The season-wide fixture list is cached for 5 minutes and shared across every
    // lookup the app makes, so try it before searchDirect — which costs several
    // fresh per-team API calls on every single search. A slip with several legs
    // resolving live tracking at once previously fired a burst of those; now they
    // all share one (cached) season fetch instead, which is much friendlier to the
    // shared rate limit on the free provider key.
    const seasonFixtures = await this.getSeasonFixtures(season)
    const cachedMatches = this.filterFixtures(seasonFixtures, normalizedQuery)
    if (cachedMatches.length) return cachedMatches

    if (!normalizedQuery) return []

    // Falls through only for matches outside the 6 tracked leagues, or not yet in
    // the season cache.
    return this.filterFixtures(await this.searchDirect(query), normalizedQuery)
  }
