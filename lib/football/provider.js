...
export class FootballProvider {
  ...
  async getMatchEvents(id) {
    ...
    // For "First Goalscorer" — same "own goals don't count" rule PP's own market
    // blurb states, so this is the first credited scorer, not just the first goal.
    const firstScorer = scorerEvents[0]?.participants?.[0]?.athlete?.displayName || null
