...
// Generic club-type words used interchangeably as either a prefix or a suffix
// (Paddy Power's "FC Sabah" vs ESPN's "Sabah FK") — stripping them as whole words
// (never mid-word) lets the two otherwise-identical names line up.
const CLUB_TOKENS = new Set(['fc', 'cf', 'fk', 'sk', 'sc', 'ac', 'afc', 'cfc', 'ssc'])
function stripClubTokens(name) {
  return name
    .split(' ')
    .filter((word) => !CLUB_TOKENS.has(word))
    .join(' ')
    .trim()
}
...
export function teamNamesMatch(left, right) {
  ...
  const strippedA = stripClubTokens(a)
  const strippedB = stripClubTokens(b)
  if (!strippedA || !strippedB) return false
  return strippedA === strippedB || strippedA.includes(strippedB) || strippedB.includes(strippedA)
}
